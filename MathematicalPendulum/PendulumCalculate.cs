...
namespace MathematicalPendulum
{
    struct PointD
    {
        ...
        public PointD(double angle_, double speed_, double time_, double acceleration_, double force_)
        {
            this.angle = angle_;
            this.speed = speed_;
            this.time = time_;
    ...
    struct RKresult
    {
        ...
        public RKresult(double t_, double x_, double Vx_, double Acc_, double F_)
        {
            this.t = t_;
            this.x = x_;
            this.Vx = Vx_;
    ...
    class PendulumCalculate
    {
        ...
        {
            double CurAng, CurAngSpeed, time_, Acc_, F_;
            CurAng = CurrentAngle;
            ...
            {
                data.Add(new PointD(CurAng, CurAngSpeed, time_, Acc, F));
                CurAng -= 0.2;
                CurAngSpeed -= 0.2;
            }
        }

        public double[] PhaseCalculate(double angle, double speed, double time, double acceleration, double force)
        {
        ...
        {
            double k1a, k1v, k2a, k2v, k3a, k3v, k4a, k4v, acceleration;
            ...
            //acceleration = Ae-dt*Sin(корень(G/L)*T)+что то похожее на альфа
            ...
            RKresult koor4 = new RKresult(tNew, aNew, vNew, AccNew, FNew);
            return koor4;
        }
    }
}
